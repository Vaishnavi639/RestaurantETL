...
def setup_with_verification(context):
    ...
    context.log.info(f"✓ PDF exists at: {os.path.abspath(pdf_path)}")
    ...
    try:
        ...
        if industry == 'restaurant':
            context.log.info(f"✓ Correct industry type for menu extraction")
        else:
            context.log.warning(f"  Business is '{industry}', not 'restaurant'")
